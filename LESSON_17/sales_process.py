...
from settings.setting import query_sales_bronze_to_silver_upd, gsc_settings, folders
import shutil
import logging
...
path = folders['raw_dir_sales']

SCHEMA_LIST = []
TABLE = 'Sales'
TABLE_ID = ProjectId+'.bronze.Sales'
...
@dag(
    ...
    catchup = False,
    max_active_runs = 1,
    tags = ['csv_load', 'bronze','silver','sales']
...
def sales_process():
    ...
    def move_files_to_gc_bronze(file_path_inf):
        ...
        f_gcs = file_path[0:10]
        folder =str('data')+'/'+str('sales')+'/'+str(f_gcs)
        ...
        for i in os.listdir(path):
            if os.path.isdir(path + '\\' + i):
                ...
                for file in list_dir:
                    file_ = file 
                #move_files_to_gc(folder, file_)
                file_path_inf.append(folder)
                file_path_inf.append(file_)
        return(file_path_inf)
              
    #load data from bronze to silver dataset
    @task(task_id='sales_bronze_to_silver')
    def sales_bronze_to_silver():

        query = query_sales_bronze_to_silver_upd
    ...
    set_schema() >> move_files_to_gc_bronze(listFoldersFiles()) >> sales_bronze_to_silver()
